...
function getDetailData() {
    ...
    const rslt = decor.find(product => product.id === +queryId);
...
const rslt = decor.filter(product => product.category.includes(queryCategory));
console.log(rslt);

function createProductBlock(rslt) {
    const productData = '<li class = "item" category="' +
        rslt.category + '"company="' +
        rslt.company + '"><a href = "./decor_detail.html?id=' +
        rslt.id +
        '&category=' +
        rslt.category +
        '"> <span> <img src = "' +
